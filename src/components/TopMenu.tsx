...
//import { getProductsNumberOnCartAction } from '@/shopping-cart/actions/actions'
import { cookies } from 'next/headers';
import Link from 'next/link'
import { CiChat1, CiMenuBurger, CiSearch, CiShoppingBasket } from 'react-icons/ci'
import { getTotalCountProducts } from '@/shopping-cart/helpers/server-helpers';
...
export const TopMenu = () => {
    ...
    const totalItems = getTotalCountProducts(cart);

    return (
        <div className="sticky w-full z-10 top-0 h-16 border-b bg-white  lg:py-2.5">
            <div className="px-6 flex items-center justify-between space-x-4">
                ...
                <div className="flex space-x-2">
                    ...
                    <Link 
                        href={'/dashboard/cart'}
                        className=" p-2 flex items-center justify-center hover:bg-slate-200  h-10 rounded-xl border bg-gray-100 focus:bg-gray-100 active:bg-gray-200">
                        <span className='text-sm mr-2 text-blue-600 font-bold'>
                            { /* getProductsNumberOnCartAction() */ }
                            { totalItems }
                        </span>
